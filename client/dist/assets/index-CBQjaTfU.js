(function(){let e=document.createElement(`link`).relList;if(e&&e.supports&&e.supports(`modulepreload`))return;for(let e of document.querySelectorAll(`link[rel="modulepreload"]`))n(e);new MutationObserver(e=>{for(let t of e)if(t.type===`childList`)for(let e of t.addedNodes)e.tagName===`LINK`&&e.rel===`modulepreload`&&n(e)}).observe(document,{childList:!0,subtree:!0});function t(e){let t={};return e.integrity&&(t.integrity=e.integrity),e.referrerPolicy&&(t.referrerPolicy=e.referrerPolicy),e.crossOrigin===`use-credentials`?t.credentials=`include`:e.crossOrigin===`anonymous`?t.credentials=`omit`:t.credentials=`same-origin`,t}function n(e){if(e.ep)return;e.ep=!0;let n=t(e);fetch(e.href,n)}})();var e=new class{constructor(){this.routes={},this.currentRoute=null,this.beforeEach=null,window.addEventListener(`hashchange`,()=>this.handleRoute()),window.addEventListener(`load`,()=>this.handleRoute())}on(e,t,n={}){return this.routes[e]={handler:t,options:n},this}guard(e){return this.beforeEach=e,this}navigate(e){window.location.hash=`#/${e}`}getPath(){return(window.location.hash.slice(2)||`login`).split(`?`)[0]}getParams(){let e=(window.location.hash.slice(2)||``).split(`?`)[1]||``;return Object.fromEntries(new URLSearchParams(e))}async handleRoute(){let e=this.getPath(),t=this.routes[e];if(!t){let e=localStorage.getItem(`ep_token`);this.navigate(e?`dashboard`:`login`);return}if(this.beforeEach&&!await this.beforeEach(e,t.options))return;let n=document.getElementById(`app`);if(n)try{n.classList.remove(`page-enter`),n.offsetWidth,n.classList.add(`page-enter`),n.innerHTML=await t.handler(this.getParams()),window.dispatchEvent(new CustomEvent(`route:loaded`,{detail:{path:e,params:this.getParams()}})),this.currentRoute=e}catch(e){console.error(`Route error:`,e),n.innerHTML=`
    ...
    <aside id="sidebar" class="fixed left-0 top-0 h-full w-64 z-40 flex flex-col transition-transform duration-300 lg:translate-x-0 -translate-x-full" style="background: linear-gradient(180deg, #1a2e4a 0%, #0f1d31 100%);">
      ...
      <nav class="flex-1 px-3 py-4 space-y-0.5 overflow-y-auto">
        ${d.filter(e=>e.roles.includes(n)).map(e=>`
          <a href="#/${e.id}" class="sidebar-link ${r===e.id?`active`:``}" data-page="${e.id}">
            ${e.icon}
      ...
      <div class="mx-3 mb-4 p-3 rounded-lg" style="background: rgba(255,255,255,0.05); border: 1px solid rgba(255,255,255,0.06);">
        <a href="#/profile" class="flex items-center gap-3 group cursor-pointer mb-2.5">
          ...
          <div class="flex-1 min-w-0">
            <p class="text-white text-sm font-medium truncate group-hover:text-accent-300 transition-colors leading-tight">${t?.full_name||`Foydalanuvchi`}</p>
            <p class="text-slate-500 text-xs">${{admin:`Administrator`,pedagog:`Pedagog`,mehmon:`Mehmon`}[n]||n}</p>
          </div>
        </a>
  ...
  `}function p(){let t=document.getElementById(`logout-btn`);t&&t.addEventListener(`click`,()=>{i.logout(),e.navigate(`login`)});let n=document.getElementById(`mobile-menu-toggle`),r=document.getElementById(`sidebar`);n&&r&&n.addEventListener(`click`,()=>{r.classList.toggle(`-translate-x-full`)})}var ee=`modulepreload`,m=function(e){return`/`+e},h={},g=function(e,t,n){let r=Promise.resolve();if(t&&t.length>0){let e=document.getElementsByTagName(`link`),i=document.querySelector(`meta[property=csp-nonce]`),a=i?.nonce||i?.getAttribute(`nonce`);function o(e){return Promise.all(e.map(e=>Promise.resolve(e).then(e=>({status:`fulfilled`,value:e}),e=>({status:`rejected`,reason:e}))))}r=o(t.map(t=>{if(t=m(t,n),t in h)return;h[t]=!0;let r=t.endsWith(`.css`),i=r?`[rel="stylesheet"]`:``;if(n)for(let n=e.length-1;n>=0;n--){let i=e[n];if(i.href===t&&(!r||i.rel===`stylesheet`))return}else if(document.querySelector(`link[href="${t}"]${i}`))return;let o=document.createElement(`link`);if(o.rel=r?`stylesheet`:ee,r||(o.as=`script`),o.crossOrigin=``,o.href=t,a&&o.setAttribute(`nonce`,a),document.head.appendChild(o),r)return new Promise((e,n)=>{o.addEventListener(`load`,e),o.addEventListener(`error`,()=>n(Error(`Unable to preload CSS for ${t}`)))})}))}function i(e){let t=new Event(`vite:preloadError`,{cancelable:!0});if(t.payload=e,window.dispatchEvent(t),!t.defaultPrevented)throw e}return r.then(t=>{for(let e of t||[])e.status===`rejected`&&i(e.reason);return e().catch(i)})},_=null,v=null,y=null,b=[0,0,0,0,0,0,0],x=30;function S(e){return String(e).padStart(2,`0`)}function C(){let e=new Date;return`${S(e.getHours())}:${S(e.getMinutes())}:${S(e.getSeconds())}`}function te(){return new Date().toLocaleDateString(`uz-UZ`,{weekday:`long`,year:`numeric`,month:`long`,day:`numeric`})}function w(e,t){if(!e)return;let n=Math.ceil(t/(800/16)),r=0,i=setInterval(()=>{r=Math.min(r+n,t),e.textContent=r,r>=t&&clearInterval(i)},16)}function T(e){let t=Math.floor((Date.now()-new Date(e))/1e3);return t<60?`${t}s oldin`:t<3600?`${Math.floor(t/60)}m oldin`:t<86400?`${Math.floor(t/3600)}s oldin`:new Date(e).toLocaleDateString(`uz-UZ`)}var E={create:{text:`Yukladi`,color:`#10b981`,dot:`bg-emerald-400`},update:{text:`Yangiladi`,color:`#3b82f6`,dot:`bg-blue-400`},delete:{text:`O'chirdi`,color:`#ef4444`,dot:`bg-red-400`},login:{text:`Kirdi`,color:`#8b5cf6`,dot:`bg-violet-400`}},D={document:`hujjat`,portfolio:`portfolio`,user:`foydalanuvchi`},O={uploaded:{label:`Yuklangan`,cls:`bg-emerald-50 text-emerald-700 border-emerald-200`},pending:{label:`Kutilmoqda`,cls:`bg-amber-50 text-amber-700 border-amber-200`},overdue:{label:`Muddati o'tgan`,cls:`bg-red-50 text-red-700 border-red-200`}};async function ne(){let e=i.getUser(),t={totalDocuments:0,todayUploads:0,overdueDocuments:0,totalPedagog:0,weeklyActivity:b},n=[],r=[];try{let[e,i,a]=await Promise.all([u.get(`/monitoring/stats`).catch(()=>t),u.get(`/documents`).catch(()=>({data:[]})),u.get(`/monitoring/activity`).catch(()=>({data:[]}))]);t=e,b=e.weeklyActivity||b,n=(i.data||[]).slice(0,6),r=(a.data||[]).slice(0,12)}catch(e){console.error(`Dashboard load error:`,e)}return`
      ...
      <main class="flex-1 lg:ml-64 flex flex-col min-h-screen">
        ...
        <div class="sticky top-0 z-30 bg-white border-b border-gray-200 px-6 py-3 flex items-center justify-between">
          <div class="flex items-center gap-3">
            ...
            <div>
              ...
              <p class="text-xs text-gray-400" id="live-date">${te()}</p>
            </div>
          </div>
          ...
          <div class="flex items-center gap-4">
            ...
            <div class="hidden sm:flex items-center gap-2 text-xs text-gray-400">
              ...
              <span id="refresh-countdown">${x}s</span>
            </div>
            <!-- Live clock -->
            <div class="font-mono text-sm font-bold text-gray-700 bg-gray-100 px-3 py-1 rounded-lg tracking-wider" id="live-clock">${C()}</div>
            <!-- Salom -->
            <div class="hidden sm:flex items-center gap-2">
              ...
              <span class="text-sm font-medium text-gray-700">${e?.full_name?.split(` `)[0]||`Foydalanuvchi`}</span>
            </div>
          </div>
        </div>
        ...
        <div class="flex-1 p-5 lg:p-6 space-y-5">
          ...
          <div class="grid grid-cols-2 lg:grid-cols-4 gap-4">
            ${[{id:`stat-docs`,label:`Jami hujjatlar`,val:t.totalDocuments,trend:`+3 bugun`,color:`#1d4ed8`,bg:`#eff6ff`,icon:`M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z`,trendUp:!0},{id:`stat-today`,label:`Bugungi yuklamalar`,val:t.todayUploads,trend:`Faol`,color:`#0d9488`,bg:`#f0fdfa`,icon:`M7 16a4 4 0 01-.88-7.903A5 5 0 1115.9 6L16 6a5 5 0 011 9.9M15 13l-3-3m0 0l-3 3m3-3v12`,trendUp:!0},{id:`stat-overdue`,label:`Muddati o'tgan`,val:t.overdueDocuments,trend:`Diqqat!`,color:`#dc2626`,bg:`#fef2f2`,icon:`M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-2.5L13.732 4c-.77-.833-1.964-.833-2.732 0L4.082 16.5c-.77.833.192 2.5 1.732 2.5z`,trendUp:!1},{id:`stat-pedagog`,label:`Faol pedagoglar`,val:t.totalPedagog,trend:`Onlayn`,color:`#7c3aed`,bg:`#f5f3ff`,icon:`M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z`,trendUp:!0}].map(e=>`
              <div class="bg-white rounded-xl border border-gray-200 p-4 hover:shadow-md transition-shadow">
                <div class="flex items-center justify-between mb-3">
                  <span class="text-xs font-medium text-gray-500 uppercase tracking-wide">${e.label}</span>
                  <div class="w-8 h-8 rounded-lg flex items-center justify-center" style="background:${e.bg}; color:${e.color}">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="${e.icon}"/></svg>
                  </div>
                </div>
                <div class="text-2xl font-bold text-gray-900 mb-1" id="${e.id}">${e.val}</div>
                <div class="flex items-center gap-1 text-xs ${e.trendUp?`text-emerald-600`:`text-red-500`}">
                  <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="${e.trendUp?`M5 10l7-7m0 0l7 7m-7-7v18`:`M19 14l-7 7m0 0l-7-7m7 7V3`}"/></svg>
                  ${e.trend}
                </div>
              </div>
            `).join(``)}
            ...
            <div class="xl:col-span-2 bg-white rounded-xl border border-gray-200 overflow-hidden">
              <div class="flex items-center justify-between px-5 py-3 border-b border-gray-100">
                <div class="flex items-center gap-2">
                  ...
                  <span class="text-xs text-gray-400 bg-gray-100 px-2 py-0.5 rounded-full" id="doc-count">${n.length}</span>
                </div>
  ...
  `}function k(e){return e.length?e.map(e=>{let t=E[e.action]||{text:e.action,color:`#64748b`,dot:`bg-gray-400`},n=D[e.entity_type]||e.entity_type;return`
      <div class="flex items-start gap-2.5 px-4 py-2.5 hover:bg-gray-50 transition-colors">
        <div class="mt-1.5 w-1.5 h-1.5 rounded-full flex-shrink-0 ${t.dot}"></div>
        <div class="flex-1 min-w-0">
          <p class="text-xs text-gray-700 leading-snug">
            <span class="font-medium">${e.user_name||`Foydalanuvchi`}</span>
            <span style="color:${t.color}"> ${t.text}</span>
            <span class="text-gray-500"> — ${n}</span>
          </p>
          <p class="text-[11px] text-gray-400 mt-0.5">${T(e.created_at)}</p>
        </div>
      </div>`}).join(``):`<div class="px-4 py-6 text-center text-xs text-gray-400">Faollik yo'q</div>`}function A(e){if(!e.length)return`<div class="px-5 py-8 text-center text-xs text-gray-400">Hujjatlar yo'q</div>`;let t={pdf:`#ef4444`,docx:`#3b82f6`,doc:`#3b82f6`,xlsx:`#10b981`,xls:`#10b981`,pptx:`#f59e0b`,ppt:`#f59e0b`};return`
    <table class="w-full text-xs">
      ...
      <tbody>
        ${e.map(e=>{let n=O[e.status]||O.pending,r=t[e.file_type]||`#64748b`;return`
          <tr class="border-t border-gray-50 hover:bg-gray-50 transition-colors">
            <td class="px-5 py-3">
              <div class="flex items-center gap-2.5">
                <span class="text-[10px] font-bold px-1.5 py-0.5 rounded" style="background:${r}18; color:${r};">${(e.file_type||``).toUpperCase()}</span>
                <span class="font-medium text-gray-800 truncate max-w-[180px]">${e.title}</span>
              </div>
            </td>
            <td class="px-5 py-3 text-gray-500">${e.author_name||`—`}</td>
            <td class="px-5 py-3 text-gray-500">${e.category}</td>
            <td class="px-5 py-3"><span class="text-[11px] font-medium px-2 py-0.5 rounded-full border ${n.cls}">${n.label}</span></td>
            <td class="px-5 py-3 text-gray-400 font-mono">${T(e.created_at)}</td>
          </tr>`}).join(``)}
      </tbody>
    </table>`}function re(){p(),P(),j(),M()}function j(){_&&clearInterval(_),_=setInterval(()=>{let e=document.getElementById(`live-clock`);e?e.textContent=C():clearInterval(_)},1e3)}function M(){let e=x;v&&clearInterval(v),v=setInterval(()=>{e--;let t=document.getElementById(`refresh-countdown`);if(!t){clearInterval(v);return}t.textContent=`${e}s`,e<=0&&(e=x,N())},1e3)}async function N(){try{let[e,t,n]=await Promise.all([u.get(`/monitoring/stats`).catch(()=>null),u.get(`/documents`).catch(()=>null),u.get(`/monitoring/activity`).catch(()=>null)]);if(e&&(w(document.getElementById(`stat-docs`),e.totalDocuments),w(document.getElementById(`stat-today`),e.todayUploads),w(document.getElementById(`stat-overdue`),e.overdueDocuments),w(document.getElementById(`stat-pedagog`),e.totalPedagog),e.weeklyActivity&&y&&(y.data.datasets[0].data=e.weeklyActivity,y.update(`active`))),t?.data){let e=t.data.slice(0,6),n=document.getElementById(`recent-docs-table`);n&&(n.innerHTML=A(e));let r=document.getElementById(`doc-count`);r&&(r.textContent=e.length)}if(n?.data){let e=document.getElementById(`activity-feed`);e&&(e.innerHTML=k(n.data.slice(0,12)));let t=document.getElementById(`feed-updated`);t&&(t.textContent=`${new Date().toLocaleTimeString(`uz-UZ`)} yangilandi`)}let r=document.getElementById(`last-refresh`);r&&(r.textContent=new Date().toLocaleTimeString(`uz-UZ`))}catch(e){console.error(`Refresh error:`,e)}}function P(){let e=document.getElementById(`activity-chart`);e&&g(async()=>{let{Chart:e,registerables:t}=await import(`./chart-vM59ydkj.js`);return{Chart:e,registerables:t}},[]).then(({Chart:t,registerables:n})=>{t.register(...n);let r=t.getChart(e);r&&r.destroy(),y=new t(e,{type:`bar`,data:{labels:[`Dush`,`Sesh`,`Chor`,`Pay`,`Jum`,`Shan`,`Yak`],datasets:[{label:`Hujjatlar`,data:b,backgroundColor:e=>{let t=e.chart.ctx.createLinearGradient(0,0,0,200);return t.addColorStop(0,`rgba(29, 78, 216, 0.7)`),t.addColorStop(1,`rgba(29, 78, 216, 0.1)`),t},borderColor:`rgba(29, 78, 216, 0.9)`,borderWidth:2,borderRadius:6,borderSkipped:!1}]},options:{responsive:!0,maintainAspectRatio:!1,animation:{duration:600},plugins:{legend:{display:!1},tooltip:{backgroundColor:`#1e293b`,titleColor:`#94a3b8`,bodyColor:`#f1f5f9`,padding:10,borderColor:`#334155`,borderWidth:1,callbacks:{label:e=>` ${e.parsed.y} ta hujjat`}}},scales:{y:{beginAtZero:!0,grid:{color:`rgba(0,0,0,0.04)`},border:{display:!1},ticks:{font:{family:`Inter`,size:11},stepSize:1,color:`#94a3b8`}},x:{grid:{display:!1},border:{display:!1},ticks:{font:{family:`Inter`,size:11},color:`#94a3b8`}}}}})})}function F(e,t=``){let n=new Date().toLocaleDateString(`uz-UZ`,{weekday:`long`,year:`numeric`,month:`long`,day:`numeric`});return`
    <header class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8 pb-6 border-b border-gray-200">
      <div class="flex items-center gap-3">
        ...
        <div>
          <h1 class="text-xl font-bold text-dark-800 leading-tight">${e}</h1>
          <p class="text-dark-400 text-xs mt-0.5">${t||n}</p>
        </div>
      </div>
      <div class="flex items-center gap-2.5">
  ...
  `}function I(e,t,n={}){let{onConfirm:r,onCancel:i,confirmText:a=`Tasdiqlash`,cancelText:o=`Bekor qilish`,size:s=`md`}=n,c={sm:`max-w-sm`,md:`max-w-lg`,lg:`max-w-2xl`,xl:`max-w-4xl`},l=document.createElement(`div`);l.id=`modal-overlay`,l.className=`modal-overlay`,l.innerHTML=`
  ...
  `,document.body.appendChild(l);let u=()=>{l.style.opacity=`0`,l.style.transition=`opacity 0.2s ease-out`,setTimeout(()=>l.remove(),200)};l.querySelector(`#modal-close-btn`).addEventListener(`click`,()=>{i&&i(),u()}),l.addEventListener(`click`,e=>{e.target===l&&(i&&i(),u())});let d=l.querySelector(`#modal-cancel-btn`);d&&d.addEventListener(`click`,()=>{i&&i(),u()});let f=l.querySelector(`#modal-confirm-btn`);f&&f.addEventListener(`click`,()=>{r&&r(u)});let p=e=>{e.key===`Escape`&&(i&&i(),u(),document.removeEventListener(`keydown`,p))};return document.addEventListener(`keydown`,p),{close:u,overlay:l}}var L=[`Barchasi`,`Dars ishlanma`,`Taqvim reja`,`Hisobot`,`O'quv dastur`,`Metodik qo'llanma`,`Boshqa`],R={uploaded:{label:`Yuklangan`,class:`badge-success`},pending:{label:`Kutilmoqda`,class:`badge-warning`},overdue:{label:`Muddati o'tgan`,class:`badge-danger`}},z={pdf:{color:`text-red-500 bg-red-50`,label:`PDF`},docx:{color:`text-primary-500 bg-primary-50`,label:`DOCX`},doc:{color:`text-primary-500 bg-primary-50`,label:`DOC`},xlsx:{color:`text-accent-500 bg-accent-50`,label:`XLSX`},xls:{color:`text-accent-500 bg-accent-50`,label:`XLS`},pptx:{color:`text-amber-500 bg-amber-50`,label:`PPTX`},ppt:{color:`text-amber-500 bg-amber-50`,label:`PPT`}};function B(e){return e?e<1024?e+` B`:e<1048576?(e/1024).toFixed(1)+` KB`:(e/1048576).toFixed(1)+` MB`:`—`}async function V(){let e=[];try{e=(await u.get(`/documents`)).data||[]}catch(e){console.error(`Initial documents load error:`,e),o(`Hujjatlarni yuklashda xatolik yuz berdi.`,`error`)}return`
    <div class="flex min-h-screen bg-dark-50">
      ${f()}
      <main class="flex-1 lg:ml-64 p-6 lg:p-8">
        ${F(`Hujjatlar ombori`,`Barcha hujjatlarni boshqaring va saralang`)}
  ...
  `}function H(e){let t=z[e.file_type]||{color:`text-dark-500 bg-dark-50`,label:e.file_type?.toUpperCase()||`FAYL`},n=R[e.status]||{label:e.status,class:`badge-warning`},r=i.getUser(),a=i.hasRole(`admin`)||i.hasRole(`pedagog`)&&e.uploaded_by===r.id;return`
    <tr class="border-t border-dark-100 hover:bg-primary-50/50 transition-colors">
      ...
      <td class="text-dark-500">${B(e.file_size)}</td>
      ...
      <td>
    ...
    </tr>`}async function U(){let e=document.getElementById(`doc-search`)?.value||``,t=document.getElementById(`doc-category-filter`)?.value||`Barchasi`,n=document.getElementById(`doc-status-filter`)?.value||``,r=document.getElementById(`documents-table-body`);if(r)try{let i=`/documents?search=${encodeURIComponent(e)}&category=${encodeURIComponent(t)}`;n&&(i+=`&status=${encodeURIComponent(n)}`);let a=(await u.get(i)).data||[];a.length===0?r.innerHTML=`
        <tr>
          <td colspan="8" class="text-center py-8 text-dark-400 font-medium">Hujjatlar topilmadi</td>
        </tr>`:r.innerHTML=a.map(e=>H(e)).join(``);let o=document.getElementById(`total-count`);o&&(o.textContent=a.length)}catch(e){console.error(`Table reload error:`,e)}}function W(){p();let e=document.getElementById(`upload-btn`);e&&e.addEventListener(`click`,()=>{I(`Yangi hujjat yuklash`,`
        <form id="upload-form" class="space-y-4">
          ...
          <div>
            ...
            <select class="input" id="upload-category">${L.slice(1).map(e=>`<option>${e}</option>`).join(``)}</select>
          </div>
          <div>
    ...
    <div class="flex min-h-screen bg-dark-50">
      ...
      <main class="flex-1 lg:ml-64 p-6 lg:p-8">
        ...
        <div id="portfolio-grid" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
          ...
          `:t.map((e,t)=>J(e,t)).join(``)}
        </div>
      </main>
    </div>
  `}function J(e,t=0){let n=G[e.type]||{label:e.type,class:`badge-info`,icon:`📝`},r=i.getUser(),a=i.hasRole(`admin`)||e.user_id===r.id,o=e.issue_date?new Date(e.issue_date).toLocaleDateString(`uz-UZ`):`—`;return`
    <div class="glass-card p-5 hover:shadow-xl hover:-translate-y-1 transition-all duration-300 portfolio-item-card animate-scale-in" data-type="${e.type}" style="animation-delay: ${t*60}ms">
    ...
    </div>`}async function Y(e=`Barchasi`){let t=i.getUser(),n=document.getElementById(`portfolio-grid`);if(n)try{let r=`/portfolio/${t.id}`;e!==`Barchasi`&&(r+=`?type=${e}`);let i=(await u.get(r)).data||[];if(i.length===0?n.innerHTML=`
        <div class="col-span-full text-center py-12 text-dark-400 font-medium bg-white rounded-3xl border border-dark-100">
          Portfolio yozuvlari mavjud emas.
        </div>`:n.innerHTML=i.map((e,t)=>J(e,t)).join(``),e===`Barchasi`){let e=i.filter(e=>e.type===`sertifikat`).length,t=i.filter(e=>e.type===`yutuq`).length,n=i.filter(e=>e.type===`ishlanma`).length,r=document.getElementById(`count-certs`),a=document.getElementById(`count-yutuq`),o=document.getElementById(`count-ishlanma`);r&&(r.textContent=e),a&&(a.textContent=t),o&&(o.textContent=n)}}catch(e){console.error(`Grid reload error:`,e)}}function ie(){p(),document.querySelectorAll(`.portfolio-filter`).forEach(e=>{e.addEventListener(`click`,()=>{document.querySelectorAll(`.portfolio-filter`).forEach(e=>{e.className=`portfolio-filter px-4 py-2 rounded-xl text-sm font-medium transition-all whitespace-nowrap bg-white text-dark-500 hover:bg-dark-100 border border-dark-200`}),e.className=`portfolio-filter px-4 py-2 rounded-xl text-sm font-medium transition-all whitespace-nowrap bg-primary-500 text-white shadow-lg shadow-primary-500/25`;let t=e.dataset.type;Y(t)})});let e=document.getElementById(`add-portfolio-btn`);e&&e.addEventListener(`click`,()=>{I(`Yangi yozuv qo'shish`,`
      ...
      `,{confirmText:`Saqlash`,onConfirm:async e=>{let t=document.getElementById(`portfolio-title`)?.value.trim(),n=document.getElementById(`portfolio-type`)?.value,r=document.getElementById(`portfolio-desc`)?.value.trim(),i=document.getElementById(`portfolio-date`)?.value;if(!t||!n){o(`Sarlavha va tur majburiy`,`warning`);return}let a=document.querySelector(`.modal-confirm-btn`);a&&(a.disabled=!0,a.textContent=`Saqlanmoqda...`);try{await u.post(`/portfolio`,{title:t,type:n,description:r,issue_date:i||null}),o(`Portfolio yozuvi qo'shildi!`,`success`),e(),Y()}catch(e){o(e.message||`Saqlashda xatolik yuz berdi`,`error`),a&&(a.disabled=!1,a.textContent=`Saqlash`)}}})});let t=document.getElementById(`portfolio-grid`);t&&t.addEventListener(`click`,async e=>{let t=e.target.closest(`.delete-portfolio-btn`),n=e.target.closest(`.view-portfolio-file`);if(t){let e=t.dataset.id;if(confirm(`Ushbu yozuvni o'chirishni xohlaysizmi?`))try{await u.delete(`/portfolio/${e}`),o(`Portfolio yozuvi o'chirildi`,`success`),Y()}catch(e){o(e.message||`O'chirishda xatolik yuz berdi`,`error`)}}if(n){let e=n.dataset.link;e&&window.open(e,`_blank`)}})}var X=[0,0,0,0,0,0,0],Z={excellent:{label:`A'lo`,dot:`status-online`,bg:`bg-accent-100 text-accent-700`},good:{label:`Yaxshi`,dot:`status-online`,bg:`bg-primary-100 text-primary-700`},warning:{label:`O'rtacha`,dot:`status-warning`,bg:`bg-amber-100 text-amber-700`},danger:{label:`Past`,dot:`status-offline`,bg:`bg-red-100 text-red-700`}};async function ae(){let e={totalDocuments:0,totalUsers:0,totalPedagog:0,todayUploads:0,overdueDocuments:0,weeklyActivity:[0,0,0,0,0,0,0],teachersStats:[]};try{let t=await u.get(`/monitoring/stats`);e=t,X=t.weeklyActivity||[0,0,0,0,0,0,0]}catch(e){console.error(`Fetch monitoring stats error:`,e)}let t=e.teachersStats||[],n=t.reduce((e,t)=>e+t.uploaded,0),r=t.reduce((e,t)=>e+t.required,0)||1,i=Math.round(n/r*100),a=t.filter(e=>e.status===`excellent`).length,o=t.filter(e=>e.status===`danger`||e.status===`warning`).length;return`
    <div class="flex min-h-screen bg-dark-50">
      ${f()}
      <main class="flex-1 lg:ml-64 overflow-y-auto">
        <div class="p-6 lg:p-8">
        ${F(`Monitoring paneli`,`Pedagoglar faoliyatini kuzating`)}
        ...
        <div class="grid grid-cols-1 xl:grid-cols-3 gap-6 mb-6">
          ...
          <div class="glass-card p-6">
            ...
            <div class="space-y-4">
              ${Object.entries(Z).map(([e,n])=>{let r=t.filter(t=>t.status===e).length,i=t.length>0?Math.round(r/t.length*100):0;return`
                <div>
          ...
          <div class="overflow-x-auto">
            <table class="w-full text-sm mt-4">
              ...
              <tbody>
                ...
                `:t.map(e=>{let t=Math.round(e.uploaded/e.required*100),n=Z[e.status]||Z.danger;return`
                  <tr class="border-t border-dark-100 hover:bg-primary-50/50 transition-colors">
                    <td class="px-6 py-4">
                      <div class="flex items-center gap-3">
  ...
  `}function oe(){p(),se()}function se(){let e=document.getElementById(`monitoring-chart`);e&&g(async()=>{let{Chart:e,registerables:t}=await import(`./chart-vM59ydkj.js`);return{Chart:e,registerables:t}},[]).then(({Chart:t,registerables:n})=>{t.register(...n);let r=t.getChart(e);r&&r.destroy(),new t(e,{type:`line`,data:{labels:[`Dush`,`Sesh`,`Chor`,`Pay`,`Jum`,`Shan`,`Yak`],datasets:[{label:`Yuklangan`,data:X,borderColor:`#3b82f6`,backgroundColor:`rgba(59,130,246,0.1)`,fill:!0,tension:.4,borderWidth:2,pointBackgroundColor:`#3b82f6`,pointBorderColor:`#fff`,pointBorderWidth:2,pointRadius:5,pointHoverRadius:7}]},options:{responsive:!0,maintainAspectRatio:!1,interaction:{intersect:!1,mode:`index`},plugins:{legend:{display:!1},tooltip:{backgroundColor:`rgba(15, 23, 42, 0.9)`,titleFont:{family:`Inter`,size:13},bodyFont:{family:`Inter`,size:12},padding:12,cornerRadius:10,displayColors:!0}},scales:{y:{beginAtZero:!0,grid:{color:`rgba(0,0,0,0.04)`},ticks:{font:{family:`Inter`,size:11},stepSize:1}},x:{grid:{display:!1},ticks:{font:{family:`Inter`,size:11}}}}}})}).catch(e=>{console.error(`Chart.js failed to load:`,e)})}var ce={admin:`Administrator`,pedagog:`Pedagog`,mehmon:`Mehmon`},le={admin:`bg-violet-100 text-violet-700`,pedagog:`bg-primary-100 text-primary-700`,mehmon:`bg-dark-100 text-dark-600`};async function ue(){let e=[];try{e=(await u.get(`/users`)).data||[]}catch(e){console.error(`Fetch users error:`,e),o(`Foydalanuvchilar ro'yxatini yuklab bo'lmadi.`,`error`)}return`
    <div class="flex min-h-screen bg-dark-50">
      ${f()}
      <main class="flex-1 lg:ml-64 p-6 lg:p-8">
        ${F(`Foydalanuvchilar`,`Tizim foydalanuvchilarini boshqaring`)}
  ...
  `}function Q(e){let t=i.getUser()?.id===e.id;return`
    <tr class="border-t border-dark-100 hover:bg-primary-50/50 transition-colors">
      <td class="px-6 py-4">
        <div class="flex items-center gap-3">
      ...
      <td class="px-6 py-4"><span class="badge ${le[e.role]}">${ce[e.role]}</span></td>
      <td class="px-6 py-4 text-dark-600">${e.subject||`—`}</td>
    ...
    </tr>`}async function $(){let e=document.getElementById(`user-search`)?.value||``,t=document.getElementById(`users-table-body`);if(t)try{let n=(await u.get(`/users?search=${encodeURIComponent(e)}`)).data||[];n.length===0?t.innerHTML=`
        <tr>
          <td colspan="7" class="text-center py-8 text-dark-400 font-medium">Foydalanuvchilar topilmadi</td>
        </tr>`:t.innerHTML=n.map(e=>Q(e)).join(``);let r=document.getElementById(`total-users-count`),i=document.getElementById(`pedagog-users-count`),a=document.getElementById(`admin-users-count`);r&&(r.textContent=n.length),i&&(i.textContent=n.filter(e=>e.role===`pedagog`).length),a&&(a.textContent=n.filter(e=>e.role===`admin`).length)}catch(e){console.error(`Table reload error:`,e)}}function de(){p(),document.getElementById(`user-search`)?.addEventListener(`input`,$),document.getElementById(`add-user-btn`)?.addEventListener(`click`,()=>{I(`Yangi foydalanuvchi yaratish`,`
    ...
    `,{confirmText:`Saqlash`,onConfirm:async e=>{let t=document.getElementById(`user-fullname`)?.value.trim(),n=document.getElementById(`user-email`)?.value.trim(),r=document.getElementById(`user-password`)?.value,i=document.getElementById(`user-role`)?.value,a=document.getElementById(`user-subject`)?.value.trim(),s=document.getElementById(`user-phone`)?.value.trim();if(!t||!n||!r||!i){o(`Barcha majburiy maydonlarni to'ldiring`,`warning`);return}let c=document.querySelector(`.modal-confirm-btn`);c&&(c.disabled=!0,c.textContent=`Saqlanmoqda...`);try{await u.post(`/users`,{full_name:t,email:n,password:r,role:i,subject:i===`pedagog`?a:null,phone:s}),o(`Foydalanuvchi muvaffaqiyatli yaratildi!`,`success`),e(),$()}catch(e){o(e.message||`Xatolik yuz berdi`,`error`),c&&(c.disabled=!1,c.textContent=`Saqlash`)}}})});let e=document.getElementById(`users-table-body`);e&&e.addEventListener(`click`,async e=>{let t=e.target.closest(`.delete-user-btn`),n=e.target.closest(`.edit-user-btn`);if(t){let e=t.dataset.id;if(confirm(`Ushbu foydalanuvchini tizimdan butunlay o'chirib tashlamoqchimisiz?`))try{await u.delete(`/users/${e}`),o(`Foydalanuvchi muvaffaqiyatli o'chirildi`,`success`),$()}catch(e){o(e.message||`O'chirishda xatolik yuz berdi`,`error`)}}if(n){let{id:e,fullname:t,email:r,role:i,subject:a,phone:s,active:c}=n.dataset,l=c===`true`;I(`Foydalanuvchini tahrirlash`,`
