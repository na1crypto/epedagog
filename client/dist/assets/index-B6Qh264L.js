(function(){let e=document.createElement(`link`).relList;if(e&&e.supports&&e.supports(`modulepreload`))return;for(let e of document.querySelectorAll(`link[rel="modulepreload"]`))n(e);new MutationObserver(e=>{for(let t of e)if(t.type===`childList`)for(let e of t.addedNodes)e.tagName===`LINK`&&e.rel===`modulepreload`&&n(e)}).observe(document,{childList:!0,subtree:!0});function t(e){let t={};return e.integrity&&(t.integrity=e.integrity),e.referrerPolicy&&(t.referrerPolicy=e.referrerPolicy),e.crossOrigin===`use-credentials`?t.credentials=`include`:e.crossOrigin===`anonymous`?t.credentials=`omit`:t.credentials=`same-origin`,t}function n(e){if(e.ep)return;e.ep=!0;let n=t(e);fetch(e.href,n)}})();var e=new class{constructor(){this.routes={},this.currentRoute=null,this.beforeEach=null,window.addEventListener(`hashchange`,()=>this.handleRoute()),window.addEventListener(`load`,()=>this.handleRoute())}on(e,t,n={}){return this.routes[e]={handler:t,options:n},this}guard(e){return this.beforeEach=e,this}navigate(e){window.location.hash=`#/${e}`}getPath(){return(window.location.hash.slice(2)||`login`).split(`?`)[0]}getParams(){let e=(window.location.hash.slice(2)||``).split(`?`)[1]||``;return Object.fromEntries(new URLSearchParams(e))}async handleRoute(){let e=this.getPath(),t=this.routes[e];if(!t){let e=localStorage.getItem(`ep_token`);this.navigate(e?`dashboard`:`login`);return}if(this.beforeEach&&!await this.beforeEach(e,t.options))return;let n=document.getElementById(`app`);if(n)try{n.classList.remove(`page-enter`),n.offsetWidth,n.classList.add(`page-enter`),n.innerHTML=await t.handler(this.getParams()),window.dispatchEvent(new CustomEvent(`route:loaded`,{detail:{path:e,params:this.getParams()}})),this.currentRoute=e}catch(e){console.error(`Route error:`,e),n.innerHTML=`
  ...
  `}function p(){let t=document.getElementById(`logout-btn`);t&&t.addEventListener(`click`,()=>{i.logout(),e.navigate(`login`)});let n=document.getElementById(`mobile-menu-toggle`),r=document.getElementById(`sidebar`);n&&r&&n.addEventListener(`click`,()=>{r.classList.toggle(`-translate-x-full`)})}function m(e,t=``){let n=new Date().toLocaleDateString(`uz-UZ`,{weekday:`long`,year:`numeric`,month:`long`,day:`numeric`});return`
  ...
  `}var h=`modulepreload`,g=function(e){return`/`+e},_={},v=function(e,t,n){let r=Promise.resolve();if(t&&t.length>0){let e=document.getElementsByTagName(`link`),i=document.querySelector(`meta[property=csp-nonce]`),a=i?.nonce||i?.getAttribute(`nonce`);function o(e){return Promise.all(e.map(e=>Promise.resolve(e).then(e=>({status:`fulfilled`,value:e}),e=>({status:`rejected`,reason:e}))))}r=o(t.map(t=>{if(t=g(t,n),t in _)return;_[t]=!0;let r=t.endsWith(`.css`),i=r?`[rel="stylesheet"]`:``;if(n)for(let n=e.length-1;n>=0;n--){let i=e[n];if(i.href===t&&(!r||i.rel===`stylesheet`))return}else if(document.querySelector(`link[href="${t}"]${i}`))return;let o=document.createElement(`link`);if(o.rel=r?`stylesheet`:h,r||(o.as=`script`),o.crossOrigin=``,o.href=t,a&&o.setAttribute(`nonce`,a),document.head.appendChild(o),r)return new Promise((e,n)=>{o.addEventListener(`load`,e),o.addEventListener(`error`,()=>n(Error(`Unable to preload CSS for ${t}`)))})}))}function i(e){let t=new Event(`vite:preloadError`,{cancelable:!0});if(t.payload=e,window.dispatchEvent(t),!t.defaultPrevented)throw e}return r.then(t=>{for(let e of t||[])e.status===`rejected`&&i(e.reason);return e().catch(i)})},y=[0,0,0,0,0,0,0],b={doc:`<svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/></svg>`,upload:`<svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 16a4 4 0 01-.88-7.903A5 5 0 1115.9 6L16 6a5 5 0 011 9.9M15 13l-3-3m0 0l-3 3m3-3v12"/></svg>`,alert:`<svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-2.5L13.732 4c-.77-.833-1.964-.833-2.732 0L4.082 16.5c-.77.833.192 2.5 1.732 2.5z"/></svg>`,users:`<svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z"/></svg>`},x={uploaded:{label:`Yuklangan`,class:`badge-success`},pending:{label:`Kutilmoqda`,class:`badge-warning`},overdue:{label:`Muddati o'tgan`,class:`badge-danger`}},S=[{border:`#0284c7`,bg:`#eff6ff`,icon:`#0284c7`},{border:`#0d9488`,bg:`#f0fdfa`,icon:`#0d9488`},{border:`#dc2626`,bg:`#fef2f2`,icon:`#dc2626`},{border:`#7c3aed`,bg:`#f5f3ff`,icon:`#7c3aed`}];async function C(){let e=i.getUser(),t={totalDocuments:0,todayUploads:0,overdueDocuments:0,totalPedagog:0},n=[];try{let[e,r]=await Promise.all([u.get(`/monitoring/stats`).catch(e=>(console.warn(`Stats fetch failed, using fallback:`,e),{totalDocuments:0,todayUploads:0,overdueDocuments:0,totalPedagog:0,weeklyActivity:[0,0,0,0,0,0,0]})),u.get(`/documents`).catch(e=>(console.warn(`Documents fetch failed:`,e),{data:[]}))]);t=e,y=e.weeklyActivity||[0,0,0,0,0,0,0],n=(r.data||[]).slice(0,5)}catch(e){console.error(`Dashboard load error:`,e)}let r=[{label:`Jami hujjatlar`,value:t.totalDocuments,sub:`+12 bu hafta`,icon:`doc`},{label:`Bugungi yuklamalar`,value:t.todayUploads,sub:`Faol kun`,icon:`upload`},{label:`Muddati o'tganlar`,value:t.overdueDocuments,sub:`E'tibor talab etiladi`,icon:`alert`},{label:`Pedagoglar`,value:t.totalPedagog,sub:`Faol o'qituvchilar`,icon:`users`}];return`
    <div class="flex min-h-screen" style="background: #f8f9fa;">
      ...
      <main class="flex-1 lg:ml-64 p-6 lg:p-8">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5 mb-6">
          ${r.map((e,t)=>`
            <div class="stat-card animate-scale-in border-l-4" style="animation-delay: ${t*80}ms; border-left-color: ${S[t].border};">
              <div class="flex items-start justify-between">
                <div class="flex-1">
                  <p class="text-dark-400 text-xs font-medium uppercase tracking-wide">${e.label}</p>
                  <p class="text-3xl font-bold text-dark-800 mt-2 mb-1">${e.value}</p>
                  <p class="text-xs ${t===2?`text-red-500`:`text-dark-400`}">${e.sub}</p>
                </div>
                <div class="w-10 h-10 rounded-lg flex items-center justify-center flex-shrink-0 ml-3" style="background: ${S[t].bg}; color: ${S[t].icon};">
                  ${b[e.icon]}
                </div>
              </div>
            </div>
            ...
            <table class="w-full text-sm">
              ...
              <tbody>
                ...
                <tr class="border-t border-gray-100 hover:bg-gray-50 transition-colors">
                  ...
                  <td class="px-6 py-4 text-dark-500">${e.author_name||`Muallif topilmadi`}</td>
                  <td class="px-6 py-4"><span class="badge-info">${e.category}</span></td>
                  <td class="px-6 py-4 text-dark-400 text-xs">${new Date(e.created_at).toLocaleDateString(`uz-UZ`)}</td>
  ...
  `}function w(){p(),T()}function T(){let e=document.getElementById(`activity-chart`);e&&v(async()=>{let{Chart:e,registerables:t}=await import(`./chart-vM59ydkj.js`);return{Chart:e,registerables:t}},[]).then(({Chart:t,registerables:n})=>{t.register(...n);let r=t.getChart(e);r&&r.destroy(),new t(e,{type:`bar`,data:{labels:[`Dush`,`Sesh`,`Chor`,`Pay`,`Jum`,`Shan`,`Yak`],datasets:[{label:`Yuklangan hujjatlar`,data:y,backgroundColor:`rgba(2, 132, 199, 0.1)`,borderColor:`rgba(2, 132, 199, 0.7)`,borderWidth:2,borderRadius:6,borderSkipped:!1}]},options:{responsive:!0,maintainAspectRatio:!1,plugins:{legend:{display:!1}},scales:{y:{beginAtZero:!0,grid:{color:`rgba(0,0,0,0.04)`},border:{display:!1},ticks:{font:{family:`Inter`,size:11},stepSize:1,color:`#94a3b8`}},x:{grid:{display:!1},border:{display:!1},ticks:{font:{family:`Inter`,size:11},color:`#94a3b8`}}}}})})}function E(e,t,n={}){let{onConfirm:r,onCancel:i,confirmText:a=`Tasdiqlash`,cancelText:o=`Bekor qilish`,size:s=`md`}=n,c={sm:`max-w-sm`,md:`max-w-lg`,lg:`max-w-2xl`,xl:`max-w-4xl`},l=document.createElement(`div`);l.id=`modal-overlay`,l.className=`modal-overlay`,l.innerHTML=`
  ...
  `,document.body.appendChild(l);let u=()=>{l.style.opacity=`0`,l.style.transition=`opacity 0.2s ease-out`,setTimeout(()=>l.remove(),200)};l.querySelector(`#modal-close-btn`).addEventListener(`click`,()=>{i&&i(),u()}),l.addEventListener(`click`,e=>{e.target===l&&(i&&i(),u())});let d=l.querySelector(`#modal-cancel-btn`);d&&d.addEventListener(`click`,()=>{i&&i(),u()});let f=l.querySelector(`#modal-confirm-btn`);f&&f.addEventListener(`click`,()=>{r&&r(u)});let p=e=>{e.key===`Escape`&&(i&&i(),u(),document.removeEventListener(`keydown`,p))};return document.addEventListener(`keydown`,p),{close:u,overlay:l}}var D=[`Barchasi`,`Dars ishlanma`,`Taqvim reja`,`Hisobot`,`O'quv dastur`,`Metodik qo'llanma`,`Boshqa`],O={uploaded:{label:`Yuklangan`,class:`badge-success`},pending:{label:`Kutilmoqda`,class:`badge-warning`},overdue:{label:`Muddati o'tgan`,class:`badge-danger`}},k={pdf:{color:`text-red-500 bg-red-50`,label:`PDF`},docx:{color:`text-primary-500 bg-primary-50`,label:`DOCX`},doc:{color:`text-primary-500 bg-primary-50`,label:`DOC`},xlsx:{color:`text-accent-500 bg-accent-50`,label:`XLSX`},xls:{color:`text-accent-500 bg-accent-50`,label:`XLS`},pptx:{color:`text-amber-500 bg-amber-50`,label:`PPTX`},ppt:{color:`text-amber-500 bg-amber-50`,label:`PPT`}};function A(e){return e?e<1024?e+` B`:e<1048576?(e/1024).toFixed(1)+` KB`:(e/1048576).toFixed(1)+` MB`:`—`}async function j(){let e=[];try{e=(await u.get(`/documents`)).data||[]}catch(e){console.error(`Initial documents load error:`,e),o(`Hujjatlarni yuklashda xatolik yuz berdi.`,`error`)}return`
  ...
  `}function M(e){let t=k[e.file_type]||{color:`text-dark-500 bg-dark-50`,label:e.file_type?.toUpperCase()||`FAYL`},n=O[e.status]||{label:e.status,class:`badge-warning`},r=i.getUser(),a=i.hasRole(`admin`)||i.hasRole(`pedagog`)&&e.uploaded_by===r.id;return`
    <tr class="border-t border-dark-100 hover:bg-primary-50/50 transition-colors">
      ...
      <td class="text-dark-500">${A(e.file_size)}</td>
      ...
      <td>
    ...
    </tr>`}async function N(){let e=document.getElementById(`doc-search`)?.value||``,t=document.getElementById(`doc-category-filter`)?.value||`Barchasi`,n=document.getElementById(`doc-status-filter`)?.value||``,r=document.getElementById(`documents-table-body`);if(r)try{let i=`/documents?search=${encodeURIComponent(e)}&category=${encodeURIComponent(t)}`;n&&(i+=`&status=${encodeURIComponent(n)}`);let a=(await u.get(i)).data||[];a.length===0?r.innerHTML=`
        <tr>
          <td colspan="8" class="text-center py-8 text-dark-400 font-medium">Hujjatlar topilmadi</td>
        </tr>`:r.innerHTML=a.map(e=>M(e)).join(``);let o=document.getElementById(`total-count`);o&&(o.textContent=a.length)}catch(e){console.error(`Table reload error:`,e)}}function P(){p();let e=document.getElementById(`upload-btn`);e&&e.addEventListener(`click`,()=>{E(`Yangi hujjat yuklash`,`
        <form id="upload-form" class="space-y-4">
          ...
          <div>
            ...
            <select class="input" id="upload-category">${D.slice(1).map(e=>`<option>${e}</option>`).join(``)}</select>
          </div>
          <div>
  ...
  `}function R(e,t=0){let n=F[e.type]||{label:e.type,class:`badge-info`,icon:`📝`},r=i.getUser(),a=i.hasRole(`admin`)||e.user_id===r.id,o=e.issue_date?new Date(e.issue_date).toLocaleDateString(`uz-UZ`):`—`;return`
    <div class="glass-card p-5 hover:shadow-xl hover:-translate-y-1 transition-all duration-300 portfolio-item-card animate-scale-in" data-type="${e.type}" style="animation-delay: ${t*60}ms">
    ...
    </div>`}async function z(e=`Barchasi`){let t=i.getUser(),n=document.getElementById(`portfolio-grid`);if(n)try{let r=`/portfolio/${t.id}`;e!==`Barchasi`&&(r+=`?type=${e}`);let i=(await u.get(r)).data||[];if(i.length===0?n.innerHTML=`
        <div class="col-span-full text-center py-12 text-dark-400 font-medium bg-white rounded-3xl border border-dark-100">
          Portfolio yozuvlari mavjud emas.
        </div>`:n.innerHTML=i.map((e,t)=>R(e,t)).join(``),e===`Barchasi`){let e=i.filter(e=>e.type===`sertifikat`).length,t=i.filter(e=>e.type===`yutuq`).length,n=i.filter(e=>e.type===`ishlanma`).length,r=document.getElementById(`count-certs`),a=document.getElementById(`count-yutuq`),o=document.getElementById(`count-ishlanma`);r&&(r.textContent=e),a&&(a.textContent=t),o&&(o.textContent=n)}}catch(e){console.error(`Grid reload error:`,e)}}function B(){p(),document.querySelectorAll(`.portfolio-filter`).forEach(e=>{e.addEventListener(`click`,()=>{document.querySelectorAll(`.portfolio-filter`).forEach(e=>{e.className=`portfolio-filter px-4 py-2 rounded-xl text-sm font-medium transition-all whitespace-nowrap bg-white text-dark-500 hover:bg-dark-100 border border-dark-200`}),e.className=`portfolio-filter px-4 py-2 rounded-xl text-sm font-medium transition-all whitespace-nowrap bg-primary-500 text-white shadow-lg shadow-primary-500/25`;let t=e.dataset.type;z(t)})});let e=document.getElementById(`add-portfolio-btn`);e&&e.addEventListener(`click`,()=>{E(`Yangi yozuv qo'shish`,`
      ...
      `,{confirmText:`Saqlash`,onConfirm:async e=>{let t=document.getElementById(`portfolio-title`)?.value.trim(),n=document.getElementById(`portfolio-type`)?.value,r=document.getElementById(`portfolio-desc`)?.value.trim(),i=document.getElementById(`portfolio-date`)?.value;if(!t||!n){o(`Sarlavha va tur majburiy`,`warning`);return}let a=document.querySelector(`.modal-confirm-btn`);a&&(a.disabled=!0,a.textContent=`Saqlanmoqda...`);try{await u.post(`/portfolio`,{title:t,type:n,description:r,issue_date:i||null}),o(`Portfolio yozuvi qo'shildi!`,`success`),e(),z()}catch(e){o(e.message||`Saqlashda xatolik yuz berdi`,`error`),a&&(a.disabled=!1,a.textContent=`Saqlash`)}}})});let t=document.getElementById(`portfolio-grid`);t&&t.addEventListener(`click`,async e=>{let t=e.target.closest(`.delete-portfolio-btn`),n=e.target.closest(`.view-portfolio-file`);if(t){let e=t.dataset.id;if(confirm(`Ushbu yozuvni o'chirishni xohlaysizmi?`))try{await u.delete(`/portfolio/${e}`),o(`Portfolio yozuvi o'chirildi`,`success`),z()}catch(e){o(e.message||`O'chirishda xatolik yuz berdi`,`error`)}}if(n){let e=n.dataset.link;e&&window.open(e,`_blank`)}})}var V=[0,0,0,0,0,0,0],H={excellent:{label:`A'lo`,dot:`status-online`,bg:`bg-accent-100 text-accent-700`},good:{label:`Yaxshi`,dot:`status-online`,bg:`bg-primary-100 text-primary-700`},warning:{label:`O'rtacha`,dot:`status-warning`,bg:`bg-amber-100 text-amber-700`},danger:{label:`Past`,dot:`status-offline`,bg:`bg-red-100 text-red-700`}};async function U(){let e={totalDocuments:0,totalUsers:0,totalPedagog:0,todayUploads:0,overdueDocuments:0,weeklyActivity:[0,0,0,0,0,0,0],teachersStats:[]};try{let t=await u.get(`/monitoring/stats`);e=t,V=t.weeklyActivity||[0,0,0,0,0,0,0]}catch(e){console.error(`Fetch monitoring stats error:`,e)}let t=e.teachersStats||[],n=t.reduce((e,t)=>e+t.uploaded,0),r=t.reduce((e,t)=>e+t.required,0)||1,i=Math.round(n/r*100),a=t.filter(e=>e.status===`excellent`).length,o=t.filter(e=>e.status===`danger`||e.status===`warning`).length;return`
    <div class="flex min-h-screen bg-dark-50">
      ${f()}
      <main class="flex-1 lg:ml-64 overflow-y-auto">
        ...
        <div class="grid grid-cols-1 xl:grid-cols-3 gap-6 mb-6">
          ...
          <div class="glass-card p-6">
            ...
            <div class="space-y-4">
              ${Object.entries(H).map(([e,n])=>{let r=t.filter(t=>t.status===e).length,i=t.length>0?Math.round(r/t.length*100):0;return`
                <div>
          ...
          <div class="overflow-x-auto">
            <table class="w-full text-sm mt-4">
              ...
              <tbody>
                ...
                `:t.map(e=>{let t=Math.round(e.uploaded/e.required*100),n=H[e.status]||H.danger;return`
                  <tr class="border-t border-dark-100 hover:bg-primary-50/50 transition-colors">
                    <td class="px-6 py-4">
                      <div class="flex items-center gap-3">
  ...
  `}function W(){p(),G()}function G(){let e=document.getElementById(`monitoring-chart`);e&&v(async()=>{let{Chart:e,registerables:t}=await import(`./chart-vM59ydkj.js`);return{Chart:e,registerables:t}},[]).then(({Chart:t,registerables:n})=>{t.register(...n);let r=t.getChart(e);r&&r.destroy(),new t(e,{type:`line`,data:{labels:[`Dush`,`Sesh`,`Chor`,`Pay`,`Jum`,`Shan`,`Yak`],datasets:[{label:`Yuklangan`,data:V,borderColor:`#3b82f6`,backgroundColor:`rgba(59,130,246,0.1)`,fill:!0,tension:.4,borderWidth:2,pointBackgroundColor:`#3b82f6`,pointBorderColor:`#fff`,pointBorderWidth:2,pointRadius:5,pointHoverRadius:7}]},options:{responsive:!0,maintainAspectRatio:!1,interaction:{intersect:!1,mode:`index`},plugins:{legend:{display:!1},tooltip:{backgroundColor:`rgba(15, 23, 42, 0.9)`,titleFont:{family:`Inter`,size:13},bodyFont:{family:`Inter`,size:12},padding:12,cornerRadius:10,displayColors:!0}},scales:{y:{beginAtZero:!0,grid:{color:`rgba(0,0,0,0.04)`},ticks:{font:{family:`Inter`,size:11},stepSize:1}},x:{grid:{display:!1},ticks:{font:{family:`Inter`,size:11}}}}}})}).catch(e=>{console.error(`Chart.js failed to load:`,e)})}var K={admin:`Administrator`,pedagog:`Pedagog`,mehmon:`Mehmon`},q={admin:`bg-violet-100 text-violet-700`,pedagog:`bg-primary-100 text-primary-700`,mehmon:`bg-dark-100 text-dark-600`};async function J(){let e=[];try{e=(await u.get(`/users`)).data||[]}catch(e){console.error(`Fetch users error:`,e),o(`Foydalanuvchilar ro'yxatini yuklab bo'lmadi.`,`error`)}return`
    <div class="flex min-h-screen bg-dark-50">
      ${f()}
      <main class="flex-1 lg:ml-64 p-6 lg:p-8">
  ...
  `}function Y(e){let t=i.getUser()?.id===e.id;return`
    <tr class="border-t border-dark-100 hover:bg-primary-50/50 transition-colors">
      <td class="px-6 py-4">
        <div class="flex items-center gap-3">
      ...
      <td class="px-6 py-4"><span class="badge ${q[e.role]}">${K[e.role]}</span></td>
      <td class="px-6 py-4 text-dark-600">${e.subject||`—`}</td>
    ...
    </tr>`}async function X(){let e=document.getElementById(`user-search`)?.value||``,t=document.getElementById(`users-table-body`);if(t)try{let n=(await u.get(`/users?search=${encodeURIComponent(e)}`)).data||[];n.length===0?t.innerHTML=`
        <tr>
          <td colspan="7" class="text-center py-8 text-dark-400 font-medium">Foydalanuvchilar topilmadi</td>
        </tr>`:t.innerHTML=n.map(e=>Y(e)).join(``);let r=document.getElementById(`total-users-count`),i=document.getElementById(`pedagog-users-count`),a=document.getElementById(`admin-users-count`);r&&(r.textContent=n.length),i&&(i.textContent=n.filter(e=>e.role===`pedagog`).length),a&&(a.textContent=n.filter(e=>e.role===`admin`).length)}catch(e){console.error(`Table reload error:`,e)}}function Z(){p(),document.getElementById(`user-search`)?.addEventListener(`input`,X),document.getElementById(`add-user-btn`)?.addEventListener(`click`,()=>{E(`Yangi foydalanuvchi yaratish`,`
    ...
    `,{confirmText:`Saqlash`,onConfirm:async e=>{let t=document.getElementById(`user-fullname`)?.value.trim(),n=document.getElementById(`user-email`)?.value.trim(),r=document.getElementById(`user-password`)?.value,i=document.getElementById(`user-role`)?.value,a=document.getElementById(`user-subject`)?.value.trim(),s=document.getElementById(`user-phone`)?.value.trim();if(!t||!n||!r||!i){o(`Barcha majburiy maydonlarni to'ldiring`,`warning`);return}let c=document.querySelector(`.modal-confirm-btn`);c&&(c.disabled=!0,c.textContent=`Saqlanmoqda...`);try{await u.post(`/users`,{full_name:t,email:n,password:r,role:i,subject:i===`pedagog`?a:null,phone:s}),o(`Foydalanuvchi muvaffaqiyatli yaratildi!`,`success`),e(),X()}catch(e){o(e.message||`Xatolik yuz berdi`,`error`),c&&(c.disabled=!1,c.textContent=`Saqlash`)}}})});let e=document.getElementById(`users-table-body`);e&&e.addEventListener(`click`,async e=>{let t=e.target.closest(`.delete-user-btn`),n=e.target.closest(`.edit-user-btn`);if(t){let e=t.dataset.id;if(confirm(`Ushbu foydalanuvchini tizimdan butunlay o'chirib tashlamoqchimisiz?`))try{await u.delete(`/users/${e}`),o(`Foydalanuvchi muvaffaqiyatli o'chirildi`,`success`),X()}catch(e){o(e.message||`O'chirishda xatolik yuz berdi`,`error`)}}if(n){let{id:e,fullname:t,email:r,role:i,subject:a,phone:s,active:c}=n.dataset,l=c===`true`;E(`Foydalanuvchini tahrirlash`,`
